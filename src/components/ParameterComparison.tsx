...
const ParameterComparison = ({ customMaterials }: ParameterComparisonProps) => {
  ...
  const addScenario = () => {
    if (scenarios.length < 4) {
      setScenarios([
        ...
        {
          id: crypto.randomUUID(),
          name: `Senaryo ${String.fromCharCode(65 + scenarios.length)}`,
          materialId: allMaterials[0].id,
          toolId: toolTypes[1].id,
          diameter: 20,
          depth: 2,
  ...
  const calculateMetrics = (scenario: Scenario) => {
    ...
    return {
      ...
      mrr: mrr.toFixed(2),
      toolLife: toolLife.toFixed(1),
      power: power.toFixed(2),
      speedEfficiency: speedEfficiency.toFixed(0),
      lifeScore: lifeScore.toFixed(0),
      mrrScore: mrrScore.toFixed(0),
  ...
  const comparisonData = useMemo(() => {
    return scenarios.map((s) => {
      ...
      return {
        name: s.name,
        spindleSpeed: metrics.spindleSpeed,
        tableFeed: metrics.tableFeed,
        mrr: parseFloat(metrics.mrr),
        toolLife: parseFloat(metrics.toolLife),
        power: parseFloat(metrics.power),
        score: parseFloat(metrics.overallScore),
      };
    });
  }, [scenarios]);
  ...
  const radarData = useMemo(() => {
    return [
      { metric: "Hız", ...Object.fromEntries(scenarios.map((s) => [s.name, parseFloat(calculateMetrics(s).speedEfficiency)])) },
      { metric: "Ömür", ...Object.fromEntries(scenarios.map((s) => [s.name, parseFloat(calculateMetrics(s).lifeScore)])) },
      { metric: "MRR", ...Object.fromEntries(scenarios.map((s) => [s.name, parseFloat(calculateMetrics(s).mrrScore)])) },
      { metric: "Güç Eff.", ...Object.fromEntries(scenarios.map((s) => [s.name, Math.max(0, 100 - parseFloat(calculateMetrics(s).power) * 20)])) },
    ];
  }, [scenarios]);

  const bestScenario = useMemo(() => {
    let best = scenarios[0];
    let bestScore = parseFloat(calculateMetrics(scenarios[0]).overallScore);
    
    scenarios.forEach((s) => {
      const score = parseFloat(calculateMetrics(s).overallScore);
      if (score > bestScore) {
        best = s;
        bestScore = score;
      }
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    className="bg-transparent border-none text-sm font-medium text-foreground focus:outline-none w-24"
                  />
              ...
              <div className="space-y-2 text-xs">
                <div>
                  <label className="text-muted-foreground">Malzeme</label>
                  <select
                    value={scenario.materialId}
                    onChange={(e) => updateScenario(scenario.id, "materialId", e.target.value)}
                    className="input-industrial w-full text-xs py-1"
                  >
                    {allMaterials.map((m) => (
                ...
                <div>
                  <label className="text-muted-foreground">Takım</label>
                  <select
                    value={scenario.toolId}
                    onChange={(e) => updateScenario(scenario.id, "toolId", e.target.value)}
                    className="input-industrial w-full text-xs py-1"
                  >
                    {toolTypes.map((t) => (
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="text-muted-foreground">Çap (mm)</label>
                    <input
                      type="number"
                      value={scenario.diameter}
                      onChange={(e) => updateScenario(scenario.id, "diameter", Number(e.target.value))}
                      className="input-industrial w-full text-xs py-1"
                    />
                  </div>
                  <div>
                    <label className="text-muted-foreground">Hız (m/dk)</label>
                    <input
                      type="number"
                      value={scenario.cuttingSpeed}
                      onChange={(e) => updateScenario(scenario.id, "cuttingSpeed", Number(e.target.value))}
                      className="input-industrial w-full text-xs py-1"
                    />
                  </div>
                </div>
        ...
        <div className="p-4 rounded-lg bg-card border border-border">
          ...
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={comparisonData}>
              ...
              />
              <Legend />
              <Bar dataKey="mrr" name="MRR (cm³/dk)" fill="hsl(var(--primary))" />
              <Bar dataKey="toolLife" name="Ömür (dk)" fill="hsl(var(--accent))" />
            </BarChart>
          </ResponsiveContainer>
        </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border">
              <th className="text-left py-3 px-4 text-muted-foreground font-medium">Metrik</th>
              {scenarios.map((s, i) => (
          ...
          <tbody>
            {[
              { label: "Devir (dev/dk)", key: "spindleSpeed", icon: Zap },
              { label: "İlerleme (mm/dk)", key: "tableFeed", icon: TrendingUp },
              { label: "MRR (cm³/dk)", key: "mrr", icon: TrendingUp },
              { label: "Takım Ömrü (dk)", key: "toolLife", icon: Clock },
              { label: "Güç (kW)", key: "power", icon: Zap },
            ].map((row) => (
              <tr key={row.key} className="border-b border-border/50 hover:bg-secondary/30">
                ...
                {scenarios.map((s) => {
                  const metrics = calculateMetrics(s);
                  const value = metrics[row.key as keyof typeof metrics];
                  return (
                    <td key={s.id} className="text-center py-3 px-4 font-mono text-foreground">
                      {value}
                    </td>
                  );
                })}
            ...
            <tr className="bg-primary/5">
              <td className="py-3 px-4 font-medium text-primary">Genel Skor</td>
              {scenarios.map((s) => {
                const metrics = calculateMetrics(s);
                return (
                  <td key={s.id} className="text-center py-3 px-4 font-mono text-xl text-primary font-bold">
